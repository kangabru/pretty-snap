...
export function PatternColours() {
    ...
    return <div ref={ref} className="row flex-wrap justify-center">
...
function PatternColor({ colour, opacity }: { colour: string, opacity: number }) {
    ...
    return <button onClick={onClick} className='relative shadow rounded outline-primary m-1' style={{ backgroundColor }}
        data-target={colour == _colour && opacity == _opacity}>
        <div className="w-10 h-10 m-1 rounded-sm" style={{ backgroundColor: colour, opacity }}></div>
...
export function ColorRow() {
    ...
    return <div ref={ref} className="grid grid-rows-2 grid-flow-col gap-2 p-2 overflow-x-scroll" tabIndex={-1}>
...
function Colour({ colour }: { colour: string }) {
    ...
    return <button data-target={isTarget} onClick={onClick} className="relative in w-24 h-24 rounded outline-primary" style={{ backgroundColor: colour }}>
