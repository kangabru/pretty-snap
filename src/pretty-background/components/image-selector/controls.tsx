...
export default function ImageControls() {
    return <div className="col sm:flex-row-reverse sm:justify-between space-y-3 sm:space-y-0 sm:space-x-3">
        <QuickSearches />
        <SearchInput />
    </div>
}
...
function SearchInput() {
    ...
    const update = (e: React.KeyboardEvent<HTMLInputElement>) => e.key == "Enter"
        ? useUnsplashStore.setState({ searchTerm: inputValue ?? "" })
        : setInputValue((e.target as HTMLInputElement).value)

    return <div className={join("row justify-center sm:justify-start rounded border-b-2 pl-2 border-gray-200 focus-within:border-primary-light",
        ...
        <input disabled={isSearching} onKeyUp={update} value={inputValue ?? ""} type="text" placeholder="Search Unsplash"
            className="px-3 py-2 outline-none disabled:bg-transparent rounded transition" />
    </div>
}
