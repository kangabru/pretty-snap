...
export function QuickPresets(props: { children: any, focusArgs?: any[] }) {
...
export function QuickPreset(props: { title?: string, onClick: () => void, classes?: string, style?: CSSProperties, target?: boolean }) {
    return <button onClick={props.onClick} title={props.title} style={props.style as any} data-target={props.target}
        class={join(props.classes, "w-12 h-12 m-1 -mr-4 z-0 bg-cover rounded-full shadow border-white border-2 transform hover:scale-105 outline-primary")}>
    </button>
}
...
export function RandButton(props: { onClick: () => void }) {
    ...
    return <button className="button bg-primary-base hover:bg-primary-dark text-white rounded-full border-white border-2 shadow transform hover:scale-105"
        onClick={props.onClick} onMouseDown={setDown} onMouseUp={setUp} onKeyPress={e => e.key == "Enter" && onEnter()}>
