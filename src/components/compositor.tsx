            ...
            <div ref={dropZone} class={join("w-full", isDropping && "border-dashed border-4 rounded-xl")}>
                <label class="cursor-pointer">
                    ...
                    {dataUrl ? <img src={dataUrl} alt="Screenshot" class={CLASSES_INNER} style={stylesScreen.inner} />
                        : <div class="p-4 sm:py-8 sm:px-12 space-y-5 bg-white shadow rounded" style={stylesScreen.inner}>
...
function InfoSection({ isDropping, isError }: { isDropping: boolean, isError: boolean }) {
    return <Fragment>
        <h2 class="max-w-lg space-x-1 text-xl sm:text-3xl text-center font-open font-semibold">
            Make your snapshots
            ...
            <a href={urls.unsplash} target="blank" class="outline-link">Unsplash</a>
        </h2>
        <div class={join("w-full py-4 px-6 space-y-2 text-base sm:text-lg rounded-3xl border-4",
            isDropping ? "border-solid" : "border-dashed",
            isError ? "border-red-500" : "border-gray-400")}>
            <p class="font-semibold text-lg sm:text-2xl">Add a snapshot</p>
            {isError
